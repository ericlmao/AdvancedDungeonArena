...
public class NumberComparators {
    ...
    public static void register(@NonNull String operator, @NonNull NumberComparator comparator) {
        BY_NAME.put(comparator.getName().toLowerCase(), comparator);
        BY_OPERATOR.put(operator.toLowerCase(), comparator);
    }

    @Nullable
    public static NumberComparator getComparator(@NonNull String str) {
        NumberComparator byName = getByName(str);
        return byName == null ? getByOperator(str) : byName;
    }

    @Nullable
    public static NumberComparator getByName(@NonNull String name) {
        return BY_NAME.get(name.toLowerCase());
    }

    @Nullable
    public static NumberComparator getByOperator(@NonNull String operator) {
