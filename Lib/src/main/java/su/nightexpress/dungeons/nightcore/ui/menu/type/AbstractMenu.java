...
import su.nightexpress.dungeons.nightcore.event.MenuOpenEvent;
    ...
    protected boolean open(@NonNull Player player, @NonNull Consumer<MenuViewer> onViewSet) {
        ...
        plugin.runTask(player, () -> {
            ...
            if (view == null || viewer.isRebuildMenu()) {
                ...
                // Paper: build the view straight off the MenuType with an Adventure title.
                view = this.menuType.typed().builder().title(NightMessage.parse(title)).build(player);
