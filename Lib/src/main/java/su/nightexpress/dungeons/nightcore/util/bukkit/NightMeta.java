...
import su.nightexpress.dungeons.nightcore.util.TooltipUtil;
...
public class NightMeta implements Writeable {
    ...
    public static NightMeta fromItemStack(@NonNull ItemStack itemStack) {
        ...
        displayMeta.setHiddenComponents(TooltipUtil.getHiddenComponents(itemStack));
    ...
    public static NightMeta read(@NonNull FileConfig config, @NonNull String path) {
        ...
        if (config.getBoolean(path + ".Hide_Components", false)) {
            displayMeta.setHiddenComponents(TooltipUtil.commonComponentNamesToHide());
        }
    ...
    public void apply(@NonNull ItemStack itemStack) {
        ItemUtil.editMeta(itemStack, meta -> {
            if (meta instanceof SkullMeta skullMeta && this.skinUrl != null) {
                skullMeta.setPlayerProfile(PlayerProfiles.createStaticTexturedProfile(this.skinUrl));
            }
        ...
        if (this.hiddenComponents != null && !this.hiddenComponents.isEmpty() && !this.hideTooltip) {
            TooltipUtil.hideComponents(itemStack, this.hiddenComponents);
        }
    }
    ...
    public NightMeta hideAllComponents() {
        this.setHiddenComponents(TooltipUtil.commonComponentNamesToHide());
        return this;
    }
    ...
    public @NonNull NightMeta setPlayerProfile(@Nullable PlayerProfile profile) {
        if (profile == null) {
            return this.setSkinURL((URL) null);
        }
