...
public class BukkitThing {
    ...
    @NonNull
    private static NamespacedKey parseKeyOrThrow(@NonNull String string) {
        NamespacedKey key = NamespacedKey.fromString(string);
        if (key == null) throw new IllegalArgumentException("Invalid key: '" + string + "'");
    ...
    public static NamespacedKey parseKey(@NonNull String namespace, @NonNull String value) {
        return new NamespacedKey(namespace.toLowerCase(), value.toLowerCase());
    ...
    public static <T extends Keyed> T getByString(@NonNull RegistryType<T> registryKey, @NonNull String string) {
        NamespacedKey key = NamespacedKey.fromString(string);
        return key == null ? null : getByKey(registryKey, key);
    }
    ...
    public static <T extends Keyed> T getByNamespaceValue(@NonNull RegistryType<T> registryKey,
        ...
        NamespacedKey key = NamespacedKey.fromString(value, null);
        return key == null ? null : getByKey(registryKey, new NamespacedKey(namespace.toLowerCase(), key.getKey()));
    }
    ...
    public static <T extends Keyed> List<NamespacedKey> getKeys(@NonNull RegistryType<T> registryType) {
        ...
        return registry.keyStream().toList();
    }
