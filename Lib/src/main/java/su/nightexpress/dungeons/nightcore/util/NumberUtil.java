...
import su.nightexpress.dungeons.nightcore.util.number.CompactNumber;
...
public class NumberUtil {
    ...
    public static UniFormatter getNumberFormat() {
        return CoreSettings.NUMBER_FORMAT;
    }

    @NonNull
    public static List<NumberShortcut> getNumberShortcuts() {
        return CoreSettings.NUMBER_SHORTCUTS.stream().sorted(Comparator.comparing(NumberShortcut::getMagnitude)).toList();
    }
    ...
    public static CompactNumber asCompact(double value) {
        ...
        int step = CoreSettings.NUMBER_SHORTCUT_STEP;
        int index = -1;
